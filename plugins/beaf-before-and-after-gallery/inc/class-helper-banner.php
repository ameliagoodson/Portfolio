...
class BEAF_Helper_Banner {
    ...
    public function load_helper_banner(){
        ...
        if(!class_exists('BAFG_Before_After_Gallery_Pro')){
            ...
            add_filter('beaf_dashboard_helper_banner', [$this, 'render_helper_banner'], 10, 2);
    ...
    public function render_helper_banner() {
        ...
        if ($screen && $screen->id === 'bafg_page_beaf_settings') {
        ...
        $countdown_html = '<div class="beaf-promo-countdown" data-end-time="' . esc_attr($end_time) . '"></div>';
        ...
        <div class="beaf-sidebar-promo">
            ...
            <div class="promo-description">
                ...
                <div class="dicount-timer">
                    ...
                    <a class="discount-btn" href="<?php echo esc_url($button_url) . '?ending=' . urlencode($end_time); ?>&utm_source=banner_beaf&utm_medium=plugin_banner&utm_campaign=flash60" target="_blank" class="tf-btn tf-btn-primary">
    ...
    public function clear_helper_banner_data($campaign_id){
        ...
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'beaf_fomo_first_visit_time_'.$campaign_id);

    }

    public function beaf_admin_helper_footer_script( $screen ){
        ?>
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                const countdown = document.querySelector('.beaf-promo-countdown');
                if (!countdown) return;
